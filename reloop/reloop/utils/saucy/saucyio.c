...
void add_edge(int a, int b, int *adj, int *edg)
{
	edg[adj[a]++] = b;
	edg[adj[b]++] = a;
}
	...
	for (i = 0; i < n; ++i) {
		...
		for (j = adj[i] ; j < adj[i+1] ; j++) {
			 ...
			 */
				...
				warn("duplicate edge in input");
				printf("problem edge %d %d\n",i,edg[j]);
				for ( int k = adj[i] ; k < adj[i+1] ; k++ ) printf("(%d,%d) ", i, edg[k]);
				free(dupe_tmp);
				return 1;
			}
			dupe_tmp[edg[j]] = i+1;
