...
static void
print_coloring(struct coloring *c, int max)
{
	int i;
...
refine(struct saucy *s, struct coloring *c)
{
	...
	while (1) {
		...
		if (s->nsinduce) {
			...
			//printf("-> nonsingleton refinement: %d\n", front);
			//print_coloring(c, s->n);
		}
		else {
			return 1;
	...
	while (s->lev) {
		...
		if (descend(s, &s->right, s->start[s->lev], min)
			...
			if (s->is_automorphism(s)) {
				...
				update_theta(s);
				//printf("partition\n");
			//for (int ii = 0; ii < s->n; ii++) printf("%d ",s->theta[ii]);
	...
	int coarsest)
{
	...
	s->split = split_init;
	//print_coloring(&s->left, s->n);
	refine(s, &s->left);
	//printf("-----equitable partition done-----\n");
	//print_coloring(&s->left, s->n);
	if (coarsest) return s->left.cfront;
